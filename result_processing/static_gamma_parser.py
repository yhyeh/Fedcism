...
def get_exp_result(dataset, distr, VI, RUN, data_distr_file, result_folder,
                   legends, shard_per_user, frac, acc_threshold, fig_zoom,
                   global_ep,
    ...
    fig_size = (16*fig_zoom, 9*fig_zoom)
    ...
    for idx, run in enumerate(runs):
        ...
        # Fedcism-static
        for g in [0.1, 1.0]: # 0.1 1.0 10.0
            fname = 'algo1_r{}_e0.8'.format(g)
        ...
        for aidx, algo in enumerate(algos):
            ...
            if os.path.exists(algo_dir):
                ...
                # if aidx < len(algos)-1:
                #     ax.plot(down_sample_idx, sim_g_u, linemk[aidx], label=legends[algo], alpha=0.4, linewidth=1)
                # else:
                #     ax.plot(down_sample_idx, sim_g_u, linemk[aidx], label=legends[algo], alpha=0.9, linewidth=1)
                ...
                #     fig.savefig('{}.{}'.format(fig_path, 'eps'), format='eps', transparent=True, rasterized=True)
                # if show_fig == False:
                #     plt.close(fig)
            ...
            for x, xlab in [('epoch', 'Epoch'), ('time_simu', 'Time')]:
                ...
                for aidx, algo in enumerate(algos):
                    ...
                    if os.path.exists(algo_dir):
                        ...
                        if aidx < len(algos)-1:
                            p = ax.plot(df_result[algo][x], df_result[algo][col], linemk[aidx], label=legends[algo], alpha=0.4, linewidth=1)
                        else:
                            p = ax.plot(df_result[algo][x], df_result[algo][col], linemk[aidx], label=legends[algo], alpha=0.9, linewidth=1)
                ...
                ax.legend(fontsize=8, ncol=round(len(algos)/2), frameon=False,
                            loc='lower right')
