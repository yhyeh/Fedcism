  ...
  for RUN in 0 1 2; do
    ...
    for FRAC in 0.05; do
      ...
      python3 main_fed.py --dataset ${DATA} --model cnn --num_classes ${NCLS} --epochs ${EP} --lr ${LR} \
        ...
        --results_save ${DIR}${VI}_a${ALPHA}_r${RUN} \
        --cls_imb --vol_imb ${VI} --clsimb_type zipf  --zipf_alpha ${ALPHA}\
        --data_distr zipf_dict_users_vi${VI}_a${ALPHA}_r${RUN}.pkl
      '
      # shard 2 fed
      python3 main_fed.py --dataset ${DATA} --model cnn --num_classes ${NCLS} --epochs ${EP} --lr ${LR} \
      ...
      for ALGO in 3 0; do
        ...
        python3 main_slct_algo3.py --dataset ${DATA} --model cnn --num_classes ${NCLS} --epochs ${EP} --lr ${LR} \
          --num_users 100 --shard_per_user 10 --frac ${FRAC} --local_ep ${LE} --local_bs 50 \
          --results_save ${DIR}${VI}_a${ALPHA}_r${RUN} --myalgo ${ALGO} --deg ${DEG} --wndw_size 100 \
          --cls_imb --vol_imb ${VI} --data_distr zipf_dict_users_vi${VI}_a${ALPHA}_r${RUN}.pkl \
          --epsilon ${E} --clsimb_type zipf --zipf_alpha ${ALPHA}\
          --gpu 2
        '
        # shard 2 fed w/ sel
        python3 main_slct_algo3.py --dataset ${DATA} --model cnn --num_classes ${NCLS} --epochs ${EP} --lr ${LR} \
